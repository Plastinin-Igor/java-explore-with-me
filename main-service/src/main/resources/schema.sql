...
CREATE TABLE IF NOT EXISTS users (
	...
	email varchar(50) NOT NULL, -- Почтовый адрес
	name varchar(150) NOT NULL, -- Имя
...
CREATE TABLE IF NOT EXISTS events (
	...
	event_date time NOT NULL, -- Дата и время на которые намечено событие. Дата и время указываются в формате "yyyy-MM-dd HH:mm:ss"
	...
	request_moderatio bool DEFAULT true NULL, -- Нужна ли пре-модерация заявок на участие. Если true, то все заявки будут ожидать подтверждения инициатором события. Если false - то будут подтверждаться автоматически
	title varchar(120) NOT NULL, -- Заголовок события
	created_on time NOT NULL, -- Дата и время создания записи события
	initiator BIGINT NOT NULL, -- Пользователь
	published_on time NULL, -- Дата и время публикации события (в формате "yyyy-MM-dd HH:mm:ss")
...
COMMENT ON COLUMN events.request_moderatio IS 'Нужна ли пре-модерация заявок на участие. Если true, то все заявки будут ожидать подтверждения инициатором события. Если false - то будут подтверждаться автоматически';
...
CREATE TABLE IF NOT EXISTS participation_request (
	...
	created time NOT NULL, -- Дата и время создания заявки
